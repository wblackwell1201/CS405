...
#include <tuple>
#include <vector>
#include <regex> // including regex to scan for "or value=value" in user inputs
...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    // Using regex to scan the const for or value=value, OR value=value, utilize regex function icase to make the pattern function case insensitive
    std::regex pattern("or+\\s(0-9)+=(0-9)+|or+\\s(a-z)+=(a-z)+|or+\\s(0-9)+=(a-z)+|or+\\s(a-z)+=(0-9)+", std::regex_constants::icase);
    std::smatch match;

    // Decision branching to check for SQL injection
    if (std::regex_search(sql, match, pattern) && match.size() > 1)
    {
        std::cout << "SQL Injection attack detected" << std::endl;    // display SQL injection detection message to the terminal
        return false;
    }
